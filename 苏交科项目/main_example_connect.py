...
#server_addr = ('10.138.86.250', 20002)
server_addr = ('127.0.0.1', 20002)
...
def get_data(sock):
    ...
    recv_data = sock.recv(num)
    len_resv_data = len(recv_data)
    ###接收数据长度判断###
    while num - len_resv_data > 0:
        recv_data += sockfd.recv(num - len_resv_data)
        len_resv_data = len(recv_data)
        # print(len(recv_data))

    if len(recv_data) > 0:
        list_own = []  # 存放本船信息[经度，纬度，航向,速度,船长，船宽]
        list_ships = []  # 存放他船信息[[经度，纬度，航向,mmsi，速度],[经度，纬度，航向,mmsi，速度]]
        # print('对方发送的信息：/n', str(recv_data, encoding='utf-8'))
        string_line = recv_data.splitlines()
        for one_line in string_line:
            line = one_line.split(b',')
            # print(line, line[0][:6])
            if line[0] == b'$!NDOS:OwnShip':
                ownship_lon = float(line[1])  # 本船经度
                ownship_lat = float(line[2])  # 本船纬度
                ownship_sog = float(line[3])  # 本船速度
                ownship_cog = float(line[4])  # 本船航向
                ownship_lenth = float(line[5])  # 本船船长
                ownship_width = float(line[6])  # 本船船宽
                list_own = [ownship_lon, ownship_lat, ownship_cog, ownship_sog, ownship_lenth, ownship_width]



            elif line[0][:7] == b'$!NDAR:':
                targetship_mmsi = int(line[0][7:])
                targetship_lon = float(line[1])
                targetship_lat = float(line[2])
                targetship_sog = float(line[3])
                targetship_cog = float(line[4])

                new_list = [targetship_lon, targetship_lat, targetship_cog, targetship_mmsi, targetship_sog]
                list_ships.append(new_list)
        return [list_own, list_ships]
    else:
        return [[], []]
...
def send_data(sock, data):
    ...
    len_data = len(data)
    byte_data = len_data.to_bytes(4, "little")
    ...
    data_resv = sock.recv(num)
    return data_resv

#从指定路径下的excel表格中获取静态信息（完成开发,验证完毕）
...
#
#             # 1)打开表
#             path="D:/code/苏交科/数据配置文档/区域数据/小船区域/正常区域/区域%d.xlsx"%(id_normalArea)
#             #path = "/ home / pi / Desktop/数据配置文档/区域数据/小船区域/正常区域/区域%d.xlsx" % (id_normalArea)
...
#             # path = "/ home / pi / Desktop/数据配置文档/区域数据/小船区域/交叉区域/区域%d.xlsx" %(id_specialArea)
#
#             data_area = xlrd.open_workbook(r"%s" % (path))
#
#             # 2）获取sheet（区域）中内容
...
class Get_StaticData():
    ...
    """
    def __init__(self):
        self.path_rel=os.path.join(os.path.dirname(os.path.abspath(__file__)),'数据配置文档')  #自适应相对路径
...
#处理坐标转换后的航道数据，划分为若干个航段（完成开发,验证完毕）
class Deal_DataChannel():
    ...
    def deal_smallChannel(self):
        ...
        for item_area in self.list_channelData[:-1]:
            ###################正常区域###################
            if item_area[0][-1][0]==1:
                channel_up01 = item_area[0]  # 上行航道1
                channel_up02 = item_area[1]  # 上行航道2
...
#判断本船/目标船与航道关系（完成开发,验证完毕）
class Which_Location():
    """
    输出："out"/"cross"/【【lon,lat】,【lon,lat】，【lon,lat】，【lon,lat】,【course，value,id1,id2】】（右上，左上，右下，左下）
    """
...
class Warm_Navigation():
    ...
    def get_warm_navigation(self):
        ...
        # print("本船与上边界距离：", s_up)
        # print("本船与下边界距离", s_down)
        # print("本船与转向点距离",s_turn)
        t_turn=s_turn/v_own
...
class Get_Most_Danger_Ship():
    ...
    def start(self,time):
        ...
        for time_moment in range(0, time, 5):
            ...
            #print(time_moment,list_own_new,list_targetShip_new)
            #####判断该时刻是否有危险船舶##########
...
###############################################一、获取坐标转换后的静态信息（区域和航道信息）（完成开发,验证完毕）##################################################
########################（一）获取小船航行时静态信息（完成开发,验证完毕）###################################
staticData_smallShip=Get_StaticData().get_data_smallShip()  #从excel表格内获取小船静态信息
staticData_smallShip_converted=Convert_Data(staticData_smallShip).convert_staticData() #转换从excel表格内获取小船静态信息的坐标
...
########################（三）处理坐标转换后的航道信息，划分为若干个航段（完成开发,验证完毕）##################
...
while True:
    t1 = time.time()
    ###############################################二、获取坐标转换后的船舶信息（完成开发,验证完毕）##############################################
    #list_ownship=  [120.088218, 31.948277, 109.46, 4.691733, 75, 12] #本船信息，使用指针从内存获取[x，y，航向,速度,船长，船宽]
    ...
    # list_ships_resv =[ [120.089587, 31.94488, 8.8, 413818205, 4]]
    ...
    # 本船处于浏河口-南京以内的正常区域/其他区域
        ...
        ########################（四）本船在航道内且顺航道行驶#########################
            ...
            data_send = "zc-in-" + danger_messege + "-" + status01 + "-" + str(s01) + "-" + status02 + "-" + str(
                s02) + "-" + status03 + "-" + C + "-" + V


    ###############################################七、解析字符串并在界面上显示（已经完成）##############################################
    data_final = send_data(sockfd, data_send)

    if data_final == b'Get Message and Prase Right':  # 判断该次传输是否正常
        continue
    else:
        print("传回的辅助决策有误!")
        break
